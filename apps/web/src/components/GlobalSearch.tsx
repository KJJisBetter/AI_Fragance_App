...
export const GlobalSearch: React.FC<GlobalSearchProps> = ({ isOpen, onClose }) => {
  ...
  const {
    ...
    start: startVoice,
    stop: stopVoice,
  } = useVoiceSearch({ continuous: false })

  // Update query as speech is transcribed
  useEffect(() => {
    if (transcript) {
      setQuery(transcript)
    }
  ...
  const trackAndSaveSearch = useCallback(
    (searchTerm: string, resultCount: number = 0) => {
  ...
  // Search fragrances
  const searchFragrances = useCallback(
          ...
          {voiceSupported && (
            <button
              ...
              className={`p-1 rounded-md transition-colors mr-2 ${
                isListening ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-gray-100'
              }`}
          ...
          {isLoading ? (
            ...
            <div className="py-2">
              ...
              {results.map((result, index) => (
                ...
                >
                  <div className="flex-shrink-0 mr-3">{result.icon}</div>
                  <div className="flex-1 text-left">
                    <div className="font-medium text-gray-900">
                      <SearchHighlight text={result.title} searchTerm={query} className="block" />
                    </div>
