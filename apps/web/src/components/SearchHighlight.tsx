...
export const SearchHighlight: React.FC<SearchHighlightProps> = ({ text, searchTerm, className }) => {
  ...
  const regex = new RegExp(`(${searchWords.map(word =>
    ...
    <span className={className}>
      {parts.map((part, index) => {
        ...
        return isMatch ? (
          <mark
            key={index}
            className="bg-yellow-200 text-yellow-800 px-0.5 rounded-sm font-medium"
          >
            {part}
          </mark>
        ) : (
...
export const FuzzySearchHighlight: React.FC<FuzzySearchHighlightProps> = ({
  ...
  const getMatchTypeColor = (type: string) => {
    switch (type) {
      case 'exact': return 'bg-green-200 text-green-800';
      case 'partial': return 'bg-blue-200 text-blue-800';
      case 'fuzzy': return 'bg-orange-200 text-orange-800';
      case 'brand': return 'bg-purple-200 text-purple-800';
