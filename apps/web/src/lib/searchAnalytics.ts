...
class SearchAnalyticsService {
  ...
  trackSearchClick(query: string) {
    ...
    const recentSearches = this.analytics
      .filter(a => a.query === normalizedQuery)
      .slice(-5); // Get last 5 searches for this query
  ...
  private updatePopularSearches() {
    const searchCounts = new Map<string, { count: number; resultCounts: number[]; clicks: number }>();
    ...
    searchCounts.forEach((data, query) => {
      const avgResultCount = data.resultCounts.reduce((sum, count) => sum + count, 0) / data.resultCounts.length;
      ...
      this.popularSearches.set(query, {
        ...
        successRate,
        avgResultCount,
        lastSearched: Math.max(...this.analytics.filter(a => a.query === query).map(a => a.timestamp))
      });
    });
  }
  ...
  private saveToStorage() {
    try {
      ...
      localStorage.setItem(POPULAR_SEARCHES_KEY, JSON.stringify(Array.from(this.popularSearches.entries())));
    } catch (error) {
